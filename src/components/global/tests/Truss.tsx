...
const strokeColors = ['rgba(37, 35, 35, 0.3)', '#414141'];
const fillColors = ['rgba(202, 222, 252, 0.7)', '#CADEFC'];
// const textColors = ['#B8B8B8', '#4E4E4E'];
        ...
        <g id='Truss' data-name='truss'>
          ...
          {elements.map((element: IElement) => (
            <motion.path
              ...
              d={element.d}
              stroke={
                element.activeAt.includes(status)
                  ? strokeColors[1]
                  : strokeColors[0]
              }
              {...shapeVariations}
          ...
          {nodes.map((node: INode) => (
            <motion.circle
              ...
              r={node.r}
              fill={
                node.activeAt.includes(status) ? fillColors[1] : fillColors[0]
              }
              stroke={
                node.activeAt.includes(status)
                  ? strokeColors[1]
                  : strokeColors[0]
              }
              {...shapeVariations}
